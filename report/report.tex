...
lowering carbon emissions from existing equipment \citep{bdaiwi2024}. Adding
hydrogen is known to increase the laminar burning velocity and the adiabatic
flame temperature, to shorten ignition delay, and to raise the risk of
flashback, while also affecting \ce{NOx} formation \citep{li2017,bdaiwi2024}.
...
flow is set as $\dot{m} = m_\mathrm{reactor}/\tau_\mathrm{res}$, allowing the
...
hot-gas residence time is $\tau=m_\mathrm{ss}/\dot{m}$. Equilibrium-product
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\textwidth]{tad_vs_phi.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\textwidth]{reactor_model_comparison.png}
...
conditions is \SI{37.68}{\centi\meter\per\second}. This is close to the
approximately \SI{38}{\centi\meter\per\second} methane--air reference value
reported for comparable conditions \citep{li2017,turns2012}, providing a
reference consistency check on the flame-speed setup. Hydrogen enrichment
increases the calculated stoichiometric flame
speed to \SI{52.16}{\centi\meter\per\second} at \SI{40}{\percent} hydrogen in
the fuel (Figure~\ref{fig:sl}).

\begin{table}[htbp]
  \centering
  \caption{Laminar flame-speed reference consistency check at
           \SI{300}{\kelvin}, \SI{1}{\atmosphere} and $\phi=1.0$.}
  \label{tab:validation}
  \begin{tabular}{lcc}
    \toprule
    Fuel & This work [\si{\centi\meter\per\second}] &
           Reference [\si{\centi\meter\per\second}] \\
    \midrule
    \ce{CH4}/air (\SI{0}{\percent} \ce{H2})  & 37.68 & $\approx$ 38 \\
    \ce{CH4}/\ce{H2}/air (\SI{40}{\percent} \ce{H2}) & 52.16 & -- \\
    \bottomrule
  \end{tabular}
\end{table}
...
steady-state solutions on the tested residence-time grid. This ideal PSR calculation does not represent
actual engine behaviour, CFD, turbulence, combustor geometry, recirculation or
wall heat loss.
...
not exact mathematical transition points. Hydrogen enrichment moves the
...
GRI-Mech~3.0 chemistry. The present calculations show how the predicted
\ce{NO} level changes across the tested hydrogen fractions, equivalence ratios
and residence times. Detailed pathway attribution is not attempted here and
would require a separate reaction-path or sensitivity analysis
\citep{millerbowman1989}.
